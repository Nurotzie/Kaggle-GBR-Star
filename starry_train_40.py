...
#set callbacks
rlrp = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.001, patience=patience, min_delta=0.0001)
#pc = PlotterCallback()
callbacks = [
    keras.callbacks.ModelCheckpoint(
        filepath=f"resnet50-{C_Width}x{C_Height}.keras",
        save_best_only=True,
        monitor="val_loss"
    ),
    #pc,
...
kernel_size = 6
dropout = 0.6
optimizer = tf.optimizers.RMSprop(learning_rate=1e-7, momentum=0.9)
rotation = 0.1
zoom = 0.4
contrast_factor=0.6
...
#build the model including the conv base resnet
inputs = keras.Input(shape=(C_Height*Scale_Multiplier,C_Width*Scale_Multiplier,3))
x = data_augmentation(inputs)
#x = layers.Rescaling(1./255)(inputs)
x = keras.applications.resnet.preprocess_input(x)
x = conv_base(x)
